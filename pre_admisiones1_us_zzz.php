<?php
	...
	if($per == 1) {
		...
		$comienzocontrato = "2023/06/05";
	}
	else {
		$comienzocontrato = "2023/08/28";
	}
	$fincontrato = "2023/11/17";
	echo "<br> per=".$per;
	...
	$sql_beca = "SELECT * FROM tbl_becas WHERE identificacion = $documento AND periodo_lectivo = 2023";
    ...
    //echo "<br/>".$maxa;
    if($fanio == $maxa) {
        $sql_mat = "SELECT MAX(idMatricula) maxid FROM matricula WHERE date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2022-12-01'";
        //echo "<br/>".$sql_mat;
    ...
    WHERE idMatricula = (SELECT MAX(idMatricula) maxid FROM matricula WHERE date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2022-12-01')";
