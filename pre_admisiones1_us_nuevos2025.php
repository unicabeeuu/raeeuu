<?php
	...
	if($per == 1) {
		...
		$comienzocontrato = "2025/04/13";
	}
	else if($per == 3) {
		$comienzocontrato = "2025/06/08";
	}
	else {
		$comienzocontrato = "2025/08/31";
	}
	$fincontrato = "2025/11/21";
	echo "<br> per=".$per;
	...
	if($fanio == $maxa) {
		...
		WHERE idMatricula = (SELECT MAX(idMatricula) maxid FROM matricula WHERE date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2024-10-01')";
	...
	if($idest > 0) {
		...
		WHERE id_estudiante = $idest AND estado = 'pre_solicitud' AND (date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2024-10-01')";
	...
	try {
		...
		//$sheet->setCellValue('C304', $nombreA);
	...
	header('Location: https://unicab.solutions/pre_admisiones1_us_nuevos.php?documento='.$documento.'&c='.$codigo.'&msgest='.$msg_estudiante);
