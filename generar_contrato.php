<?php
	...
	AND e.n_documento = '$documento' AND m.estado = 'activo'";*/
	$sql_datos = "SELECT e.*, m.id_grado, m.n_matricula, g.grado, td.id id_td, td.tipo_documento 
	FROM estudiantes e, matricula m, grados g, tbl_tipos_documento td 
	WHERE e.id = m.id_estudiante AND m.id_grado = g.id AND e.tipo_documento = td.id 
	AND e.n_documento = '$documento'";
	echo "<br>".$sql_datos;
	...
	$sql_nuevo = "SELECT COUNT(1) ct FROM tbl_asistente_virtual av, tbl_asistente_virtual_pasos avp 
	WHERE av.paso = avp.paso AND avp.paso_numero > 300000 AND av.documento_estudiante = '$documento'";
	echo "<br>".$sql_nuevo;
	...
	echo "<br>Estudiante nuevo=".$estnuevo;
