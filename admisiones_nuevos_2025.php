<?php
	...
	//Se buscan los costos de matrícula para colegio
	/*$sql_costos = "SELECT DISTINCT c.matricula, '1° a 3°' grados FROM tbl_costos_unicab c, grados g  
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado < 5 
	UNION ALL 
	SELECT DISTINCT c.matricula, '4° y 5°' grados FROM tbl_costos_unicab c, grados g 
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado >= 5 AND c.id_grado < 6
	UNION ALL 
	SELECT DISTINCT c.matricula, '6° a 7°' grados FROM tbl_costos_unicab c, grados g 
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado >= 6 AND c.id_grado < 9
	UNION ALL 
	SELECT DISTINCT c.matricula, '8°' grados FROM tbl_costos_unicab c, grados g 
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado = 9
	UNION ALL 
	SELECT DISTINCT c.matricula, '9°, 10° y 11°' grados FROM tbl_costos_unicab c, grados g 
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado >= 10 AND c.id_grado < 13";
	$res_costos = mysqli_query($conexion, $sql_costos);
	
	//Se buscan los costos de matrícula para ciclos
	$sql_costosc = "SELECT DISTINCT c.matricula, 'CICLO I, II y III' grados FROM tbl_costos_unicab c, grados g  
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado >= 13 AND c.id_grado < 16 
	UNION ALL 
	SELECT DISTINCT c.matricula, 'CICLO IV' grados FROM tbl_costos_unicab c, grados g 
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado = 16 
	UNION ALL 
	SELECT DISTINCT c.matricula, 'CICLO V y VI' grados FROM tbl_costos_unicab c, grados g 
	WHERE c.id_grado = g.id AND c.a = 2024 AND c.id_grado >= 17 AND c.id_grado < 19";
	$res_costosc = mysqli_query($conexion, $sql_costosc);*/
...
<head>
    ...
    <script>
        ...
        function val_documento() {
            ...
            $.ajax({
				type:"POST",
				url:"cargar_datos_est_nuevo_getdat.php",
				data:"documento=" + doc,
				success:function(r) {
					...
					if(control_matricula == 0) {
						if(r_est == "activo") {
							...
							(res.apellidos == "") ? $("#ctr_register_apellidos").val(1) : $("#ctr_register_apellidos").val(0);
							(res.nombres == "") ? $("#ctr_register_nombres").val(1) : $("#ctr_register_nombres").val(0);
							//$("#ctr_register_grado").val(0);
							(res.idtdoc == "" || res.idtdoc == "0") ? $("#ctr_register_tipo_documento").val(1) : $("#ctr_register_tipo_documento").val(0);
							(res.tel == "") ? $("#ctr_register_telefono").val(1) : $("#ctr_register_telefono").val(0);
							(res.email == "") ? $("#ctr_register_email").val(1) : $("#ctr_register_email").val(0);
							(res.email == "") ? $("#ctr_register_email1").val(1) : $("#ctr_register_email1").val(0);
							(res.rh == "") ? $("#ctr_register_rh").val(1) : $("#ctr_register_rh").val(0);
							(res.idmedio == "" || res.idmedio == "0") ? $("#ctr_register_medio").val(1) : $("#ctr_register_medio").val(0);
							(res.extra == "") ? $("#ctr_activiadad_extra").val(1) : $("#ctr_activiadad_extra").val(0);
							(res.genero == "" || res.genero == "0") ? $("#ctr_register_genero").val(1) : $("#ctr_register_genero").val(0);
							...
							$("#register_genero").removeAttr("disabled");
...
<body>
    ...
    <div class="row">
		<div class="col-12">
			<center>
				<img src="assets/img/admisiones/img_nuevos_2025_1.png" id="imgh1" class="img-fluid" width="90%"/>
			</center>
		</div>
	</div><br>
    <!--== Header Area End ==-->
	...
	<div class="container" id="divCuatro">
		<div class="row">
			...
			<div class="col-lg-6 col-md-6 col-sm-6">
				<div id="divRequisitos">
					<center><h6 id="tituloRequisitos"><img src="assets/img/admisiones/flecha-96.png" alt="" class="img-fluid" style="width: 2em;"/> REQUISITOS</h6></center>
				</div>
			</div>
			<div class="col-lg-3 col-md-3 col-sm-3">				
			</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos" style="color: red;">
					<strong>Nota: Todos los documentos deben ser en pdf.</strong>
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Imagen de consignación efectuada <strong>por concepto de matrícula para el año 2025.</strong>
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Contrato de matrícula, Pagaré y consentimiento informado <span style="color: red;"><strong>firmados y autenticados en notaría</strong></span> (Archivo que se enviará Por
					correo después de diligenciar éste formulario).
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Documento de identidad del acudiente, (misma persona que firma el contrato).
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Certificado de notas <span style="color: red;"><strong>PARA INGRESAR A UN GRADO DE PRIMARIA:</strong></span> Aportar certificado de las
					notas obtenidas en el grado anterior al que aspira en el cual conste la aprobación del mismo.
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Certificado de notas <span style="color: red;"><strong>PARA INGRESAR A BACHILLERATO:</strong></span> Aportar certificado de las
					notas obtenidas en el grado 5 y hasta el anterior al que aspira en el cual conste la aprobación de los mismos.
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Paz y salvo de Colegio anterior.
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Fotocopia del certificado de la aplicación de la vacuna de Sarampión y Rubéola de niños y niñas nacidas a partir de 
					enero del 2010, según Decreto 2287 del 12 de agosto del 2003.
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Hoja de vida, observador o certificado comportamental y académico colegio anterior.
				</p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-11">
				<p class="pRequisitos">
					Certificado de actividad extracurricular.
				</p>
			</div>
		</div><br>				
	</div><br>
		...
		<div class="row">
			<div class="col-12">
				<center>
					...
					<a href="assets/descargas/costos/Circular_General_Costos2024.pdf" target="_blank" class="btn btn-brand smooth-scroll">COSTOS DE MATRICULA EXTRAORDINARIA 2024</a><br><br>
	...
	<div class="container" id="divOcho">
		...
		<form name="formulario" id="formulario" method="post" action="pre_admisiones1_us_nuevos.php" enctype="multipart/form-data">
			<div class="row ml-5">
