<?php
    ...
    if($estnuevo == "NO") {
        ...
        descuento1, tot_anual_sd, beca1, tot_anual_sb, pension_final, parentesco_acudiente_1, a_matricula, situacion_se) 
        VALUES 
        ('$apellidos', '$nombres', 'NA', '$tdoc', '$documento', '1900/01/01', 'NA', 'NA', '$dirA', 'NA', '$cel', 'NA', 
        '$extra', '$emailA', '$genero', '$nombreA', 'NA', '$celA', 'NA', '$rh', 'NA', 'NA', '$fecha2', 
        '$documentoA', $per, $descuento, $beca, $ct_pagos, $pension, 0, $pagos_anuales_de, 0, $pagos_anuales_de, $total_anual_de, 
        $descuento1, $total_anual_sd, $beca1, $total_anual_sb, $pension_final, '$parentesco1', $fanio, '$situacion')";*/
		...
		documento_responsable = '$documentoA', estado = '$rh', situacion_se = '$situacion' 
        WHERE n_documento = '$documento'";
    ...
    //echo "<br/>".$maxa;
    if($fanio == $maxa) {
        $sql_mat = "SELECT MAX(idMatricula) maxid FROM matricula WHERE date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2024-12-05'";
        //echo "<br/>".$sql_mat;
	...
	if($fanio == $maxa) {
		...
		WHERE idMatricula = (SELECT MAX(idMatricula) maxid FROM matricula WHERE date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2024-12-05')";
	...
	if($idest > 0) {
		...
		WHERE id_estudiante = $idest AND estado = 'pre_solicitud' AND (date_format(fecha_ingreso, '%Y') = $fanio OR fecha_ingreso >= '2024-12-05')";
	...
	//**************************************************************************************************************
    //Se actualiza la tabla de pre matrícula
	/*$sql_prem = "UPDATE tbl_pre_matricula 
	SET id_grado = $idgra, nombres_est = '$nombres', apellidos_est = '$apellidos', actividad_extra = '$extra', 
	nombre_a = '$nombreA', celular_a = '$celA', email_a = '$emailA', id_medio = $medio, 
	interesado = 'SI' 
	WHERE documento_est = '$documento'";
	//echo $sql_prem;
	...
	try {
		...
		$sheet->setCellValue('C214', $nombreA);
		...
		$sheet->setCellValue('C308', $nombreA);
		$sheet->setCellValue('C309', $documentoA);*/
	...
	//redireccionamos a unicab.solutions para el envío del correo a admisiones 
	//header('Location: resultado_pre_admisiones_f.php?s='.$resultado);
	//echo "<br/>página: ".'https://unicab.solutions/pre_admisiones1_us_nuevos.php?documento='.$documento.'&c='.$codigo.'&msgest='.$msg_estudiante.'&sqlinsert='.$sql_insert2;
	
	header('Location: https://unicab.solutions/pre_admisiones2_us_nuevos2025.php?documento='.$documento.'&c='.$codigo.'&msgest='.$msg_estudiante);
