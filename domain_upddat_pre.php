<?php
	...
	$sql_usu_domain = "SELECT COUNT(1) ct FROM tbl_usuarios_domain WHERE usuario = '$usu_domain'";
	//echo $sql_usu_domain;
	...
	if ($ct == 0) {
		//Se hace el registro
		$sql_insert_ini = "INSERT INTO tbl_usuarios_domain (usuario, id_palabras, fecha_registro, ultimo_id_cambiado) 
		...
		FROM tbl_usuarios_domain WHERE usuario = '$usu_domain'";
		//echo $sql_usu_domain;
		$res_usu_domain = $mysqli1->query($sql_usu_domain);
		while($row_usu_domain = $res_usu_domain->fetch_assoc()) {
		...
		if ($fecha_registro < $fecha3) {
			...
			if (count($id_palabras_array) < 25) { //25 ########################################
				...
				$sql_update = "UPDATE tbl_usuarios_domain SET id_palabras = '$id_palabras', ultimo_id_cambiado = ".$ultimo_id.",
				...
				$query1 = "SELECT MAX(id) maxidgeneral FROM tbl_metodo_domain";
				//echo $query1;					
				$resultado = $mysqli1->query($query1);
				while($row = $resultado->fetch_assoc()) {
					$maxidgeneral = $row['maxidgeneral'];
				...
				$query2 = "SELECT COUNT(1) ct FROM tbl_usuarios_domain_palabras WHERE usuario = '$usu_domain'";
				$resultado2 = $mysqli1->query($query2);
				while($row2 = $resultado2->fetch_assoc()) {
				...
				for ($i = 1; $i <= $registrosacambiar; $i++) {
					...
					if ($maxidactivo == $maxidgeneral) {
						...
						if ($totalregistros == $maxidgeneral) {
							...
							$sql_update = "UPDATE tbl_usuarios_domain SET id_palabras = '$id_palabras_final', ultimo_id_cambiado = ".$ultimo_id_cambiado.", 
							fecha_registro = '$fecha2' WHERE usuario = '$usu_domain'";
							$res_update = $mysqli1->query($sql_update);
	...
	echo "<script>location.href='domain_final.php';</script>";
