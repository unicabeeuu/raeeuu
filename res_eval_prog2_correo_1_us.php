<?php
    ...
    while($row_ctok = $exe_ctok->fetch_assoc()) {
        //echo "control";
        if($row_ctok['id_materia'] == 353) {
            $obj_json_decode['php']['ctok'] = $row_ctok['ct_ok'];
            $totphp += $row_ctok['ct_ok'];
            $total_todos_ok += $row_ctok['ct_ok'];
        }
        else if($row_ctok['id_materia'] == 354) {
            $obj_json_decode['mysql1']['ctok'] = $row_ctok['ct_ok'];
            $totmysql1 += $row_ctok['ct_ok'];
            $total_todos_ok += $row_ctok['ct_ok'];
        }
        //echo "control1";
    ...
    while($row_ctno = $exe_ctno->fetch_assoc()) {
        if($row_ctno['id_materia'] == 353) {
            $obj_json_decode['php']['ctno'] = $row_ctno['ct_no'];
            $totphp += $row_ctno['ct_no'];
        }
        else if($row_ctno['id_materia'] == 354) {
            $obj_json_decode['mysql1']['ctno'] = $row_ctno['ct_no'];
            $totmysql1 += $row_ctno['ct_no'];
        }
    ...
    while($row_ctna = $exe_ctna->fetch_assoc()) {
        if($row_ctna['id_materia'] == 353) {
            $obj_json_decode['php']['ctna'] = $row_ctna['ct_na'];
            $totphp += $row_ctna['ct_na'];
        }
        else if($row_ctna['id_materia'] == 354) {
            $obj_json_decode['mysql1']['ctna'] = $row_ctna['ct_na'];
            $totmysql1 += $row_ctna['ct_na'];
        }
    ...
    $obj_json_decode['php']['ctpen'] = $totphp;
    $obj_json_decode['mysql1']['ctpen'] = $totmysql1;
    
    //$total_todos = $tothphp + $totmysql;
	$total_todos = $totphp + $totmysql1;
    
    if($obj_json_decode['php']['ctok'] / $totphp > 0.75) {
        $nivphp = "SUPER ALTO";
        $colphp = "#138726";
    }
    else if($obj_json_decode['php']['ctok'] / $totphp > 0.5) {
        $nivphp = "ALTO";
        $colphp = "#4b9db9";
    }
    else if($obj_json_decode['php']['ctok'] / $totphp > 0.25) {
        $nivphp = "MEDIO";
        $colphp = "#FFC300";
    }
    else {
    ...
    if($obj_json_decode['mysql1']['ctok'] / $totmysql1 > 0.75) {
        $nivmysql1 = "SUPER ALTO";
        $colmysql1 = "#138726";
    }
    else if($obj_json_decode['mysql1']['ctok'] / $totmysql1 > 0.5) {
        $nivmysql1 = "ALTO";
        $colmysql1 = "#4b9db9";
    }
    else if($obj_json_decode['mysql1']['ctok'] / $totmysql1 > 0.25) {
        $nivmysql1 = "MEDIO";
        $colmysql1 = "#FFC300";
    }
    else {
                    ...
                    $content1 .= '<table id="tblglobal" style="text-align: center; padding: 0 10px;">';
                        $content1 .= '<thead style="font-size: 16px; font-weight: bold; color: #064C86;">';
                            ...
                            $content1 .= '<tr>';
                                ...
                                $content1 .= '<td width="140px" style="background: #093A5F; color: #F1F1F2; font-weight: bold; border: 2px solid black;">PHP</td>';
                                $content1 .= '<td width="140px" style="background: #093A5F; color: #F1F1F2; font-weight: bold; border: 2px solid black;">MYSQL</td>';
                        ...
                        $content1 .= '<tbody>';                            
                            ...
                            $content1 .= '<td style="border: 2px solid black; color: #093A5F; font-weight: bold;">'.$obj_json_decode['php']['ctok'].' / '.$obj_json_decode['php']['ctpen'].'</td>';
                            $content1 .= '<td style="border: 2px solid black; color: #093A5F; font-weight: bold;">'.$obj_json_decode['mysql1']['ctok'].' / '.$obj_json_decode['mysql1']['ctpen'].'</td>';
                            ...
                            $content1 .= '<td style="color: '.$colphp.'; border: 2px solid black; font-weight: bold;">'.$nivphp.'</td>';
                            $content1 .= '<td style="color: '.$colmysql1.'; border: 2px solid black; font-weight: bold;">'.$nivmysql1.'</td>';
	...
	header('location: https://unicab.solutions/res_eval_prog2_correo_1_us.php?n_documento='.$documento.'&idgra='.$idgra.'&cargo='.$grado_ra.'&pdf='.$ruta);
    echo "<script>location.href='https://unicab.solutions/res_eval_prog2_correo_1_us.php?n_documento=".$documento."&idgra=".$idgra."&cargo=".$grado_ra."&pdf=".$ruta."';</script>";
    // ###################### FIN ENVIO DE CORREO ###################
