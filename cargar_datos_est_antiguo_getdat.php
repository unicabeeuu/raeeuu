<?php
	...
	set_time_limit(300);
	//https://unicab.org/cargar_datos_est_antiguo_getdat.php?documento=1070586027
	...
	if($maxid == 0) {
		...
		e.acudiente_1, e.email_acudiente_1, e.direccion, e.telefono_acudiente_1, e.actividad_extra, e.genero, IFNULL(e.situacion_se, '') situacion_se,
	    e.documento_responsable, td.id, td.tipo_documento, e.ciudad, e.parentesco_acudiente_1 
        ...
        while($row1 = $resultado1->fetch_assoc()) {
			...
			$datos->ciudadA = $row1['ciudad'];
			$datos->parentesco = $row1['parentesco_acudiente_1'];
