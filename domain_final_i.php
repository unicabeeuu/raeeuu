<?php 
	...
	$sql_palabras = "SELECT id_palabras FROM tbl_usuarios_domain_i WHERE usuario = '$usu_domain'";
	...
	if (count($id_palabras_array) > 0) {
		$sql_delete = "DELETE FROM tbl_usuarios_domain_palabras_i WHERE usuario = '$usu_domain'";
		$exe_delete = mysqli_query($conexion, $sql_delete);
		
		for ($i = 0; $i < count($id_palabras_array); $i++) {
			$sql_insert = "INSERT INTO tbl_usuarios_domain_palabras_i (usuario, id_palabra) 
...
<head>
    ...
    <script>
		...
		function decir(palabra = "") {	
			...
			voz4.voice = speechSynthesis.getVoices()[4]; // Google US English
			voz4.text = palabra;
    ...
    <?php    
		...
		$sql_banner = "SELECT * FROM tbl_metodo_domain_i WHERE id IN (SELECT id_palabra FROM tbl_usuarios_domain_palabras_i WHERE usuario = '$usu_domain') ORDER BY id DESC";
...
<body>
    ...
    <header id="header-area">
        <?php include "header_domain_i.php"; ?>        
    </header>
    <!--== Header Area End ==-->
    ...
    <section id="slider-area">
        ...
        <div class="slider-active-wrap owl-carousel text-center text-md-left">
            ...
            <?php 
				...
				$sql_banner1="SELECT * FROM tbl_metodo_domain_i WHERE id IN (SELECT id_palabra FROM tbl_usuarios_domain_palabras_i WHERE usuario = '$usu_domain') ORDER BY id DESC";
