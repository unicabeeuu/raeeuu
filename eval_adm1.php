<?php
    ...
    if (!isset($_REQUEST['n_documento'])) {
        echo "<script>location.href='eval_adm.php'</script>";
    }
    ...
    WHERE e.id = m.id_estudiante AND m.id_grado = g.id AND e.n_documento = '$documento' AND m.estado IN ('pre_solicitud', 'solicitud', 'activo')";
	//echo $sql;
    ...
    for($i = 0; $i < $longitud_temas; $i++) {
        ...
        $exe_pregutnas_tema_soc = $mysqli1->query($sql_preguntas_tema_soc);
