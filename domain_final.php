...
<head>
    ...
    <script>
		...
		const cargarVocesEspanol = () => {
			if (vocesCargadas) return true; // Ya se cargaron
			...
			console.log('Voces aún no disponibles o no hay voces en español.');
			return false;
		};
		...
		$(function() {
			...
			// **AÑADIR ESTE LISTENER CLAVE:** Inicializa la voz con el PRIMER clic
			$(document).one('click', function() {
				// Este es el momento en que el navegador permite cargar los recursos de voz
				cargarVocesEspanol();
			});
			
			// Aseguramos que si el evento voiceschanged se dispara, se carguen las voces
			synth.onvoiceschanged = cargarVocesEspanol;
			...
			owl.on('changed.owl.carousel', function(event) {
				...
				//console.log(palabra);
				// Reintentar cargar por si el clic en el carrusel fue la primera interacción
				if (!vocesCargadas) {
					cargarVocesEspanol();
				}
				decir(palabra);
