<?php
	...
	if ($idgrado == 20) {
		header('location: eval_prog2.php?n_documento='.$documento.'&idgra='.$idgrado.'&ct_preg='.$ct_preg.'&txtgra='.$grado);
		echo "<script>location.href='eval_prog2.php?n_documento=".$documento."&idgra=".$idgrado."&ct_preg=".$ct_preg."&txtgra=".$grado."';</script>";
	}
	...
	//********************** Se cargan las preguntas de HTML5 ******************************
	$array_id_temas_html = array();
	$array_preg_tema_html = array();
    $array_preg_tema_html1 = array();
    $array_preg_html_final = array();
    $html = 0;
    
    //Se consultan los temas por pensamiento
    $sql_temas_html = "SELECT DISTINCT tp.id, tp.tema 
    ...
    $exe_temas_html = $mysqli1->query($sql_temas_html);
    while($row_temas_html = $exe_temas_html->fetch_assoc()) {
        $array_id_temas_html[$html] = $row_temas_html['id'];
        $html++;
    }
    $html = 0;
    
    //Se cargan las preguntas por cada tema
    $longitud_temas = count($array_id_temas_html);
    for($i = 0; $i < $longitud_temas; $i++) {
	    $sql_preguntas_tema_html = "SELECT id 
            FROM tbl_preguntas WHERE id_tema = $array_id_temas_html[$i] AND id_grado = $idgrado";
        $exe_preguntas_tema_html = $mysqli1->query($sql_preguntas_tema_html);
        while($row_preguntas_tema_html = $exe_preguntas_tema_html->fetch_assoc()) {
            $array_preg_tema_html[$html] = $row_preguntas_tema_html['id'];
            $array_preg_tema_html1[$html] = 0;
            $html++;
        }
        $html = 0;
    	...
    	$sql_cantidad_preg_tema = "SELECT ct_preguntas FROM tbl_temas_preguntas 
    	    WHERE id_grado = $idgrado AND id = ".$array_id_temas_html[$i];
        ...
        $control = 0;
        $longitud_preg = count($array_preg_tema_html);
        if($longitud_preg < $ct_preguntas_tema) {
            while($control < $longitud_preg) {
    	        $ale=mt_rand(1,$longitud_preg);
                if($array_preg_tema_html1[$ale-1] == 0) {
        		    $array_preg_tema_html1[$ale-1] = 1;
        		    $array_temp[$temp] = $array_preg_tema_html[$ale-1];
        		    $temp++;
            	    $control++;
        		}
    	    }
        ...
        unset($array_preg_tema_html);
        unset($array_preg_tema_html1);
	}
	$array_preg_html_final = $array_temp;
	shuffle($array_preg_html_final); //desordenar un array
	//var_export ($array_preg_bio_final);
	unset($array_temp);
	$temp = 0;
	//********************** FIN preguntas de HTML5 ******************************
	
	//********************** Se cargan las preguntas de CSS3 ******************************
	$array_id_temas_css = array();
	$array_preg_tema_css = array();
    $array_preg_tema_css1 = array();
    $array_preg_css_final = array();
    $css = 0;
    
    //Se consultan los temas por pensamiento
    $sql_temas_css = "SELECT DISTINCT tp.id, tp.tema 
        FROM tbl_preguntas p, tbl_temas_preguntas tp 
        WHERE p.id_tema = tp.id AND p.id_grado = $idgrado AND p.id_tema = ".$array_materias_final[1];
    $exe_temas_css = $mysqli1->query($sql_temas_css);
    while($row_temas_css = $exe_temas_css->fetch_assoc()) {
        $array_id_temas_css[$css] = $row_temas_css['id'];
        $css++;
    }
    $css = 0;
    
    //Se cargan las preguntas por cada tema
    $longitud_temas = count($array_id_temas_css);
    for($i = 0; $i < $longitud_temas; $i++) {
	    $sql_preguntas_tema_css = "SELECT id 
            FROM tbl_preguntas WHERE id_tema = $array_id_temas_css[$i] AND id_grado = $idgrado";
        $exe_preguntas_tema_css = $mysqli1->query($sql_preguntas_tema_css);
        while($row_preguntas_tema_css = $exe_preguntas_tema_css->fetch_assoc()) {
            $array_preg_tema_css[$css] = $row_preguntas_tema_css['id'];
            $array_preg_tema_css1[$css] = 0;
            $css++;
        }
        $css = 0;
        ...
        //Se consulta la cantidad de preguntas definidas por cada tema
    	$sql_cantidad_preg_tema = "SELECT ct_preguntas FROM tbl_temas_preguntas 
    	    WHERE id_grado = $idgrado AND id = ".$array_id_temas_css[$i];
    	$exe_cantidad_preg_tema = $mysqli1->query($sql_cantidad_preg_tema);
        while($row_cantidad_preg_tema = $exe_cantidad_preg_tema->fetch_assoc()) {
            $ct_preguntas_tema = $row_cantidad_preg_tema['ct_preguntas'];
        }
        ...
        //Se seleccionan aleatoriamente la cantidad de preguntas definidas por cada tema
        $control = 0;
        $longitud_preg = count($array_preg_tema_css);
        if($longitud_preg < $ct_preguntas_tema) {
            while($control < $longitud_preg) {
    	        $ale=mt_rand(1,$longitud_preg);
                if($array_preg_tema_css1[$ale-1] == 0) {
        		    $array_preg_tema_css1[$ale-1] = 1;
        		    $array_temp[$temp] = $array_preg_tema_css[$ale-1];
        		    $temp++;
            	    $control++;
        		}
    	    }
        ...
        //Se limpian los array
        unset($array_preg_tema_css);
        unset($array_preg_tema_css1);
	}
	$array_preg_css_final = $array_temp;
	shuffle($array_preg_css_final); //desordenar un array
	//var_export ($array_preg_bio_final);
	unset($array_temp);
	$temp = 0;
	//********************** FIN preguntas de CSS3 ******************************
	
	//********************** Se cargan las preguntas de JS ******************************
	$array_id_temas_js = array();
	$array_preg_tema_js = array();
    $array_preg_tema_js1 = array();
    $array_preg_js_final = array();
    $js = 0;
    
    //Se consultan los temas por pensamiento
    $sql_temas_js = "SELECT DISTINCT tp.id, tp.tema 
        FROM tbl_preguntas p, tbl_temas_preguntas tp 
        WHERE p.id_tema = tp.id AND p.id_grado = $idgrado AND p.id_tema = ".$array_materias_final[2];
    $exe_temas_js = $mysqli1->query($sql_temas_js);
    while($row_temas_js = $exe_temas_js->fetch_assoc()) {
        $array_id_temas_js[$js] = $row_temas_js['id'];
        $js++;
    }
    $js = 0;
    
    //Se cargan las preguntas por cada tema
    $longitud_temas = count($array_id_temas_js);
    for($i = 0; $i < $longitud_temas; $i++) {
	    $sql_preguntas_tema_js = "SELECT id 
            FROM tbl_preguntas WHERE id_tema = $array_id_temas_js[$i] AND id_grado = $idgrado";
        $exe_preguntas_tema_js = $mysqli1->query($sql_preguntas_tema_js);
        while($row_preguntas_tema_js = $exe_preguntas_tema_js->fetch_assoc()) {
            $array_preg_tema_js[$js] = $row_preguntas_tema_js['id'];
            $array_preg_tema_js1[$js] = 0;
            $js++;
        }
        $js = 0;
        ...
        //Se consulta la cantidad de preguntas definidas por cada tema
    	$sql_cantidad_preg_tema = "SELECT ct_preguntas FROM tbl_temas_preguntas 
    	    WHERE id_grado = $idgrado AND id = ".$array_id_temas_js[$i];
    	$exe_cantidad_preg_tema = $mysqli1->query($sql_cantidad_preg_tema);
        while($row_cantidad_preg_tema = $exe_cantidad_preg_tema->fetch_assoc()) {
            $ct_preguntas_tema = $row_cantidad_preg_tema['ct_preguntas'];
        }
        ...
        //Se seleccionan aleatoriamente la cantidad de preguntas definidas por cada tema
        $control = 0;
        $longitud_preg = count($array_preg_tema_js);
        if($longitud_preg < $ct_preguntas_tema) {
            while($control < $longitud_preg) {
    	        $ale=mt_rand(1,$longitud_preg);
                if($array_preg_tema_js1[$ale-1] == 0) {
        		    $array_preg_tema_js1[$ale-1] = 1;
        		    $array_temp[$temp] = $array_preg_tema_js[$ale-1];
        		    $temp++;
            	    $control++;
        		}
    	    }
        ...
        //Se limpian los array
        unset($array_preg_tema_js);
        unset($array_preg_tema_js1);
	}
	$array_preg_js_final = $array_temp;
	shuffle($array_preg_js_final); //desordenar un array
	//var_export ($array_preg_bio_final);
	unset($array_temp);
	$temp = 0;
	//********************** FIN preguntas de JS ******************************
	
	//********************** Se cargan las preguntas de HTML5CSS3 ******************************
	$array_id_temas_htmlcss = array();
	$array_preg_tema_htmlcss = array();
    $array_preg_tema_htmlcss1 = array();
    $array_preg_htmlcss_final = array();
    $htmlcss = 0;
    
    //Se consultan los temas por pensamiento
    $sql_temas_htmlcss = "SELECT DISTINCT tp.id, tp.tema 
        FROM tbl_preguntas p, tbl_temas_preguntas tp 
        WHERE p.id_tema = tp.id AND p.id_grado = $idgrado AND p.id_tema = ".$array_materias_final[3];
    $exe_temas_htmlcss = $mysqli1->query($sql_temas_htmlcss);
    while($row_temas_htmlcss = $exe_temas_htmlcss->fetch_assoc()) {
        $array_id_temas_htmlcss[$htmlcss] = $row_temas_htmlcss['id'];
        $htmlcss++;
    }
    $htmlcss = 0;
    
    //Se cargan las preguntas por cada tema
    $longitud_temas = count($array_id_temas_htmlcss);
    for($i = 0; $i < $longitud_temas; $i++) {
	    $sql_preguntas_tema_htmlcss = "SELECT id 
            FROM tbl_preguntas WHERE id_tema = $array_id_temas_htmlcss[$i] AND id_grado = $idgrado";
        $exe_preguntas_tema_htmlcss = $mysqli1->query($sql_preguntas_tema_htmlcss);
        while($row_preguntas_tema_htmlcss = $exe_preguntas_tema_htmlcss->fetch_assoc()) {
            $array_preg_tema_htmlcss[$htmlcss] = $row_preguntas_tema_htmlcss['id'];
            $array_preg_tema_htmlcss1[$htmlcss] = 0;
            $htmlcss++;
        }
        $htmlcss = 0;
        ...
        //Se consulta la cantidad de preguntas definidas por cada tema
    	$sql_cantidad_preg_tema = "SELECT ct_preguntas FROM tbl_temas_preguntas 
    	    WHERE id_grado = $idgrado AND id = ".$array_id_temas_htmlcss[$i];
    	$exe_cantidad_preg_tema = $mysqli1->query($sql_cantidad_preg_tema);
        while($row_cantidad_preg_tema = $exe_cantidad_preg_tema->fetch_assoc()) {
            $ct_preguntas_tema = $row_cantidad_preg_tema['ct_preguntas'];
        }
        ...
        //Se seleccionan aleatoriamente la cantidad de preguntas definidas por cada tema
        $control = 0;
        $longitud_preg = count($array_preg_tema_htmlcss);
        if($longitud_preg < $ct_preguntas_tema) {
            while($control < $longitud_preg) {
    	        $ale=mt_rand(1,$longitud_preg);
                if($array_preg_tema_htmlcss1[$ale-1] == 0) {
        		    $array_preg_tema_htmlcss1[$ale-1] = 1;
        		    $array_temp[$temp] = $array_preg_tema_htmlcss[$ale-1];
        		    $temp++;
            	    $control++;
        		}
    	    }
        ...
        //Se limpian los array
        unset($array_preg_tema_htmlcss);
        unset($array_preg_tema_htmlcss1);
	}
	$array_preg_htmlcss_final = $array_temp;
	shuffle($array_preg_htmlcss_final); //desordenar un array
	//var_export ($array_preg_bio_final);
	unset($array_temp);
	$temp = 0;
	//********************** FIN preguntas de HTML5CSS3 ******************************
	
	//********************** Se cargan las preguntas de JQUERY ******************************
	$array_id_temas_jq = array();
	$array_preg_tema_jq = array();
    $array_preg_tema_jq1 = array();
    $array_preg_jq_final = array();
    $jq = 0;
    
    //Se consultan los temas por pensamiento
    $sql_temas_jq = "SELECT DISTINCT tp.id, tp.tema 
        FROM tbl_preguntas p, tbl_temas_preguntas tp 
        WHERE p.id_tema = tp.id AND p.id_grado = $idgrado AND p.id_tema = ".$array_materias_final[4];
    $exe_temas_jq = $mysqli1->query($sql_temas_jq);
    while($row_temas_jq = $exe_temas_jq->fetch_assoc()) {
        $array_id_temas_jq[$jq] = $row_temas_jq['id'];
        $jq++;
    }
    $jq = 0;
    
    //Se cargan las preguntas por cada tema
    $longitud_temas = count($array_id_temas_jq);
    for($i = 0; $i < $longitud_temas; $i++) {
	    $sql_preguntas_tema_jq = "SELECT id 
            FROM tbl_preguntas WHERE id_tema = $array_id_temas_jq[$i] AND id_grado = $idgrado";
        $exe_preguntas_tema_jq = $mysqli1->query($sql_preguntas_tema_jq);
        while($row_preguntas_tema_jq = $exe_preguntas_tema_jq->fetch_assoc()) {
            $array_preg_tema_jq[$jq] = $row_preguntas_tema_jq['id'];
            $array_preg_tema_jq1[$jq] = 0;
            $jq++;
        }
        $jq = 0;
        ...
        //Se consulta la cantidad de preguntas definidas por cada tema
    	$sql_cantidad_preg_tema = "SELECT ct_preguntas FROM tbl_temas_preguntas 
    	    WHERE id_grado = $idgrado AND id = ".$array_id_temas_jq[$i];
    	$exe_cantidad_preg_tema = $mysqli1->query($sql_cantidad_preg_tema);
        while($row_cantidad_preg_tema = $exe_cantidad_preg_tema->fetch_assoc()) {
            $ct_preguntas_tema = $row_cantidad_preg_tema['ct_preguntas'];
        }
        ...
        //Se seleccionan aleatoriamente la cantidad de preguntas definidas por cada tema
        $control = 0;
        $longitud_preg = count($array_preg_tema_jq);
        if($longitud_preg < $ct_preguntas_tema) {
            while($control < $longitud_preg) {
    	        $ale=mt_rand(1,$longitud_preg);
                if($array_preg_tema_jq1[$ale-1] == 0) {
        		    $array_preg_tema_jq1[$ale-1] = 1;
        		    $array_temp[$temp] = $array_preg_tema_jq[$ale-1];
        		    $temp++;
            	    $control++;
        		}
    	    }
        ...
        //Se limpian los array
        unset($array_preg_tema_jq);
        unset($array_preg_tema_jq1);
	}
	$array_preg_jq_final = $array_temp;
	shuffle($array_preg_jq_final); //desordenar un array
	//var_export ($array_preg_bio_final);
	unset($array_temp);
	$temp = 0;
	//********************** FIN preguntas de JQUERY ******************************
    ...
    if($ct_eval_respuestas == 0) {
    	...
    	$temp = 0;
    	$control = 0;
    	$longitud_html = count($array_preg_html_final);
    	while($control < $longitud_html){
    	    $array_final[$temp] = $array_preg_html_final[$control];
    	    $temp++;
    	    $control++;
    	}
    	$control = 0;
    	$longitud_css = count($array_preg_css_final);
    	while($control < $longitud_css){
    	    $array_final[$temp] = $array_preg_css_final[$control];
    	    $temp++;
    	    $control++;
    	}
    	$control = 0;
    	$longitud_js = count($array_preg_js_final);
    	while($control < $longitud_js){
    	    $array_final[$temp] = $array_preg_js_final[$control];
    	    $temp++;
    	    $control++;
    	}
    	$control = 0;
    	$longitud_htmlcss = count($array_preg_htmlcss_final);
    	while($control < $longitud_htmlcss){
    	    $array_final[$temp] = $array_preg_htmlcss_final[$control];
    	    $temp++;
    	    $control++;
    	}
    	$control = 0;
		$longitud_jq = count($array_preg_jq_final);
    	while($control < $longitud_jq){
    	    $array_final[$temp] = $array_preg_jq_final[$control];
    	    $temp++;
    	    $control++;
    	}
    	$control = 0;
...
<head>
    ...
    <script>
        ...
        function finalizar() {
	        ...
	        location.href = 'res_eval_prog_correo_1_us.php?n_documento=' + $("#txt_documento").val() + '&idgra=' + $("#txtidgra").val();
